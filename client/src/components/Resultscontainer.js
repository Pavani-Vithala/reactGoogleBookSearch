...
function ResultsContainer(props) {
    return (
        <ul className="Results-container" style={{ borderStyle: "solid", borderWidth: 2, borderColor: "black" }}>
            {props.results.map((result, i) => {
                ...
                const { id, title, author, imageLink, bookLink, description } = result;
                return (
                    <li key={i} className="Result-item" style={{ borderStyle: "solid", borderWidth: 2, borderColor: "black" }}>

                        <h3>{title}</h3>
                        <h4>Written By {author}</h4>
                        <img alt="imageLink" src={imageLink} className="img-fluid" style={{ position: "relative" }} />
                        <p style={{ alignContent: "center", fontSize: 20 }}>{description}</p>
                        <a href={bookLink}><button className="btn btn-success"
                            style={{ margin: 10, float: "right", padding: 10 }}>View</button></a>
                        <button className="btn btn-success" id={id} onClick={(event) => { saveBookInfo(event, result) }}
    ...
    function saveBookInfo(event, bookdata) {
        ...
        API.saveBook("/api/Save", bookToSave).then(function (data) {
